...
def seat_should_flip(state, row_index, seat_index, seating_matrix):
    ...
    if seat_index > 0:
        ...
        seat = seat_index - 1
        while seat >= 0:
            seat_to_check = seating_matrix[row][seat]
            if seat_to_check != ".":
                if seat_to_check == "#":
    ...
    if row_index > 0 and seat_index < matrix_width - 1:
        row = row_index - 1
        seat = seat_index + 1
        while row >= 0 and seat <= matrix_width - 1:
            seat_to_check = seating_matrix[row][seat]
            if seat_to_check != ".":
                if seat_to_check == "#":
                    adjacent_seat_occupied_count += 1
    ...
    if row_index < matrix_height - 1 and seat_index > 0:
        row = row_index + 1
        seat = seat_index - 1
        while row <= matrix_height - 1 and seat >= 0:
            seat_to_check = seating_matrix[row][seat]
            if seat_to_check != ".":
                if seat_to_check == "#":
                    adjacent_seat_occupied_count += 1
    ...
    if row_index < matrix_height - 1 and seat_index < matrix_width - 1:
        row = row_index + 1
        seat = seat_index + 1
        while row <= matrix_height - 1 and seat <= matrix_width - 1:
            seat_to_check = seating_matrix[row][seat]
            if seat_to_check != ".":
                if seat_to_check == "#":
                    adjacent_seat_occupied_count += 1
    ...
    if state == "#" and adjacent_seat_occupied_count >= 5:
        return True
    else:
        return False
...
def count_occupied_seats(filename, is_test_data):
    if not is_test_data:
        with open(filename) as f: # "/Users/marce/AdventOfCode/2020/day_11/seating_grid.txt"
            seating_matrix = f.read().split("\n")
    else:
        seating_matrix = filename.split("\n")
