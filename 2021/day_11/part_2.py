...
def find_first_step_with_simultaneous_flash(filename):
    ...
    def step_once(grid):
        ...
        for row in range(0, len(grid)):
            for col in range(0, len(grid[row])):
                point = increase_point(row, col, grid)
                if point:
    ...
    def process_tens_or_return_step_if_all_tens(tens, grid):
        pp.pprint(grid)
        print(f"process_tens_or_return_step_if_all_tens; tens: {tens}")
        if len(tens) == len(grid) * len(grid[0]):
            return True
    ...
    while True:
        print(steps)
        tens = step_once(grid)
        step_found = process_tens_or_return_step_if_all_tens(tens, grid)
        pp.pprint(grid)
        if step_found:
            print(f"steps found: {steps - 9}")
            return
        steps += 1


find_first_step_with_simultaneous_flash("data.txt")
