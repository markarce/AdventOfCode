...
def sum_all_output_values(filename):
    ...
    for line in lines:
        ...
        for digit in output_value:
            for key in code_to_value.keys():
                if set(digit) == set(key):
                    current_value.append(str(code_to_value[key]))
                    break

        current_value = int("".join(current_value))
        output_value_sum += current_value

    print(output_value_sum)

sum_all_output_values("data.txt")